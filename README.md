...
## Push to GitHub ([hellobeekay](https://github.com/hellobeekay))

The repo must be created **while signed in as the hellobeekay account**. Your computer’s GitHub CLI (`gh`) may be logged into a *different* user—in that case GitHub will block creating `hellobeekay/koro-site` until you add or switch accounts.

### One-time: sign in as hellobeekay in the terminal

1. Install [GitHub CLI](https://cli.github.com/) if needed (`gh` command).
2. In the project folder’s terminal, run:
   ```bash
   gh auth login
   ```
3. Choose **GitHub.com** → **HTTPS** → log in with the **hellobeekay** account (browser flow is easiest).
4. If you already use another account in `gh`, switch to hellobeekay:
   ```bash
   gh auth switch -h github.com -u hellobeekay
   ```

### Create the repo and push (after `gh` is hellobeekay)

From this project’s root (where `package.json` lives):

```bash
# Remove a previous remote if you pointed at the wrong URL earlier
git remote remove origin 2>/dev/null || true

# Create hellobeekay/koro-site on GitHub and push main in one step
gh repo create hellobeekay/koro-site --public --source=. --remote=origin --push --description "Koro & Co. site — Vite/React SPA"
```

If the repo **already exists** empty on GitHub (you created it in the browser), only push:

```bash
git remote add origin https://github.com/hellobeekay/koro-site.git
git push -u origin main
```

Repo will appear at **https://github.com/hellobeekay/koro-site**.
